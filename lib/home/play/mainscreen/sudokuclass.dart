    ...
    if (_active_row != null && _active_col != null) {
      ...
      if (isdefaultCell(row, col)) {
        if (_draf == false && (_board[row][col] == 0 || _board[row][col] == num)) {
          ...
          set_board_cell_draf(row, col, num);
        }
      }
    } else {
      setnumbg(num);
  ...
  void set_board_cell_draf(int row, int col, int num, {state = null}) {
    ...
    if (_board[row][col].contains(num)) {
      ...
      if (_board[row][col].length == 1) {
        _draf = false;
        isValid(row, col, _board[row][col][0]) ? _bgcolor[row][col] = _cell_bgcolor : _bgcolor[row][col] = _errcol;
        _board[row][col] = _board[row][col][0];
        _color[row][col] = _n_color[_color[row][col][_board[row][col]]];
        print(_board[row][col]);
      } else if (_board[row][col].Empty) {
        _draf = false;
        _board[row][col] = 0;
  ...
  void set_unactive() {
    for (int row = 0; row < 9; row++) {
      for (int col = 0; col < 9; col++) {
        if (_bgcolor[row][col] == _act_bgcolor || _bgcolor[row][col] == _cell_bgcolor) {
    ...
    if (row != null && col != null) {
      if (isdefaultCell(row, col)) {
        ...
        if (_board[row][col] == 0) {
          ...
          isValid(row, col, _board[row][col]) ? _bgcolor[row][col] = _cell_bgcolor : _bgcolor[row][col] = _errcol;
          _color[row][col] = _n_color[_color[row][col][_board[row][col]]];
          _draf = false;
  ...
  void addpossible(int row, int col) {
    ...
    for (int num = 1; num < 10; num++) {
      if (isValid(row, col, num) && !_board[row][col].contains(num)) {
        set_board_cell_draf(row, col, num, state: null);
      }
    }
  }
  ...
  void clear([String state]) {
    if (_active_row != null && _active_col != null) {
      ...
      if (value != 0) {
        remove_curnum();
        _bgcolor[row][col] = _cell_bgcolor;
        String state = value is int ? 'int' : 'List';
        //changebgColor(row, col, _act_bgcolor);
        if (state == 'Undo') {
          if (value != 0) redo_push(state, row, col, value);
        } else {
  ...
  void undo() {
    if (!_undo.isEmpty()) {
      ...
      if (lundo[0] == 'int') {
        ...
        set_board_cell_draf(lundo[1], lundo[2], lundo[3]);
      }
    }
  }
  ...
  void redo() {
    if (!_redo.isEmpty()) {
      ...
      if (lredo[0] == 'int') {
        ...
        //when we clear in cell, there are two possible this cell must be specific value or draf
        setBoardCell(lredo[3], state: 'Undo');
      } else {
        //state is List,there are two possible one form list to list or one from 0 to list
        set_board_cell_draf(lredo[1], lredo[2], lredo[3]);
      }
    }
  }
